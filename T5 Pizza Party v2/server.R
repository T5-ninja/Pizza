...
server <- function(input, output) {
  ...
  # define factory dataframe
  
  values$fdf <- data.frame(
    id = as.integer(1),
    year = as.integer(0),
    tomato_bought=0,
  ...
  ##### when click button "Pizza to sell"###
  observeEvent(input$s1_pizza,{
    ...
    ##calculation for s1df
    values$vpizza_demand = as.integer(rnorm(1, mean = 8, sd=3))
    vpizza_fulfilled=min(input$vpizza_made, values$vpizza_demand)
    
    ##updating new row s1df
    newLine <- c(
      values$id,
      values$year,
      values$vpizza_demand,
      input$vpizza_made,
      vpizza_fulfilled,
      shop1_revenue=vpizza_fulfilled*vpizza
    )
  ...
  observeEvent(input$purchase,{
    ...
    newLine <- c(
      ...
      values$year,
      input$tomato_bought,
      total_ingredients_cost=input$tomato_bought*tomato
      
    )
    values$fdf[nrow(values$fdf) + 1, ] <- newLine
    values$fdf$id <- as.integer(values$fdf$id)
    values$fdf$year <- as.integer(values$fdf$year)
  }
  )
  ...
  output$f_data_out  <-
    renderTable(values$fdf[,], striped = TRUE, spacing = "xs")
  
  output$c_data_out  <-
    renderTable(values$cdf[, c(2:14)], striped = TRUE, spacing = "xs")
  output$s1_data_out  <-
    renderTable(values$s1df[,], striped = TRUE, spacing = "xs")
  
  output$downloadData <- downloadHandler(
    filename = function() {
      paste('pizzaparty', 'csv', sep = ".")
    },
